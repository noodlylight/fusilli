...
KWD::Decorator::x11EventFilter (XEvent *xevent)
{
	...
	case SelectionRequest:
		decor_handle_selection_request (QX11Info::display(), xevent, 
		                                mDmSnTimestamp);
		break;
	case SelectionClear:
...
KWD::Decorator::handleWindowAdded (WId id)
{
	...
	{
		KWindowInfo wInfo = KWindowSystem::windowInfo (id,
		                                      NET::WMWindowType, 0);
	...
	KWD::trapXError ();
	XSelectInput (QX11Info::display(), id, StructureNotifyMask | 
	                                          PropertyChangeMask);
	KWD::popXError ();
