                           ...
                           int        *gridHeight)
{
	...
	const BananaValue *
	option_horizontal_folds_num_folds = bananaGetOption (bananaIndex,
	...
	*gridWidth = 2;
	if (aw->com.curWindowEvent == WindowEventShade ||
		...
		*gridHeight = 3 + 2 *
		              option_horizontal_folds_num_folds->i;
	else
		*gridHeight = 1 + 2 *
		              option_horizontal_folds_num_folds->i;
}
                            ...
                            float      time)
{
	...
	const BananaValue *
	option_horizontal_folds_amp_mult = bananaGetOption (bananaIndex,
	                                     "horizontal_folds_amp_mult",
	                                     w->screen->screenNum);

	int nHalfFolds = 2.0 * option_horizontal_folds_num_folds->i;
	float foldMaxAmp =
	        0.3 * pow ((winHeight / nHalfFolds) / w->screen->height, 0.3) *
	        option_horizontal_folds_amp_mult->f;
...
fxHorizontalFoldsZoomToIcon (CompWindow *w)
{
	...
	const BananaValue *
	option_horizontal_folds_zoom_to_taskbar = bananaGetOption (bananaIndex,
	...
	return ((aw->com.curWindowEvent == WindowEventMinimize ||
	        ...
	        option_horizontal_folds_zoom_to_taskbar->b);
}
