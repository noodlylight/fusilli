                  ...
                  float      *thickness)
{
	...
	{
		const BananaValue *
		option_glide1_away_position = bananaGetOption (bananaIndex,
		                                               "glide1_away_position",
		                                               w->screen->screenNum);

		const BananaValue *
		option_glide1_away_angle = bananaGetOption (bananaIndex,
		                                            "glide1_away_angle",
		                                            w->screen->screenNum);

		*finalDistFac = option_glide1_away_position->f;
		*finalRotAng = option_glide1_away_angle->f;
	}
	else
	{
		const BananaValue *
		option_glide2_away_position = bananaGetOption (bananaIndex,
		                                               "glide2_away_position",
		                                               w->screen->screenNum);

		const BananaValue *
		option_glide2_away_angle = bananaGetOption (bananaIndex,
		                                            "glide2_away_angle",
		                                            w->screen->screenNum);

		*finalDistFac = option_glide2_away_position->f;
		*finalRotAng = option_glide2_away_angle->f;
	}
}
...
fxGlideZoomToIcon (CompWindow *w)
{
	...
	const BananaValue *
	option_glide1_zoom_to_taskbar = bananaGetOption (bananaIndex,
	                                      "glide1_zoom_to_taskbar",
	                                      w->screen->screenNum);

	const BananaValue *
	option_glide2_zoom_to_taskbar = bananaGetOption (bananaIndex,
	...
	return
	        ((aw->com.curWindowEvent == WindowEventMinimize ||
	         ...
	         ((aw->com.curAnimEffect == AnimEffectGlide1 &&
	           option_glide1_zoom_to_taskbar->b) ||
	          (aw->com.curAnimEffect == AnimEffectGlide2 &&
	           option_glide2_zoom_to_taskbar->b)));
}
