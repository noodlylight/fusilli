 ...
 *
 * Port to fusilli          : (C) 2015 Michail Bitzes
...
defaultAnimInit (CompWindow *w)
{
	...
	const BananaValue *
	option_time_step = bananaGetOption (bananaIndex,
	                                    "time_step",
	                                    w->screen->screenNum);
                          ...
                          int        *duration)
{
	...
	int nRows;
	const BananaValue * option_effects;
	const BananaValue * option_durations;
	CompMatch *match; //pointer to array of matches

	switch (e) {
	case AnimEventOpen:
		option_effects = bananaGetOption (bananaIndex,
		                                  "open_effects",
		                                  w->screen->screenNum);

		option_durations = bananaGetOption (bananaIndex,
		                                    "open_durations",
		                                    w->screen->screenNum);

		match = as->open_match;

		nRows = MIN (as->open_count, option_effects->list.nItem);
		nRows = MIN (nRows, option_durations->list.nItem);
		break;
	case AnimEventClose:
		option_effects = bananaGetOption (bananaIndex,
		                                  "close_effects",
		...
		match = as->close_match;

		nRows = MIN (as->close_count, option_effects->list.nItem);
		nRows = MIN (nRows, option_durations->list.nItem);
		break;
	case AnimEventMinimize:
		...
		match = as->minimize_match;

		nRows = MIN (as->minimize_count, option_effects->list.nItem);
		nRows = MIN (nRows, option_durations->list.nItem);
		break;
	case AnimEventShade:
		option_effects = bananaGetOption (bananaIndex,
		...
		match = as->shade_match;

		nRows = MIN (as->shade_count, option_effects->list.nItem);
		nRows = MIN (nRows, option_durations->list.nItem);
		break;
	case AnimEventFocus:
		option_effects = bananaGetOption (bananaIndex,
		...
		nRows = MIN (as->focus_count, option_effects->list.nItem);
		nRows = MIN (nRows, option_durations->list.nItem);
		break;
	default:
	...
	for (i = 0; i < nRows; i++)
	{
		...
		if (duration)
			*duration = option_durations->list.item[i].i;

		if (strcasecmp (option_effects->list.item[i].s, "curvedfold") == 0)
			return AnimEffectCurvedFold;

		else if (strcasecmp (option_effects->list.item[i].s, "dodge") == 0)
			return AnimEffectDodge;

		else if (strcasecmp (option_effects->list.item[i].s, "dream") == 0)
			return AnimEffectDream;

		else if (strcasecmp (option_effects->list.item[i].s, "fade") == 0)
			return AnimEffectFade;

		else if (strcasecmp (option_effects->list.item[i].s, "focusfade") == 0)
			return AnimEffectFocusFade;

		else if (strcasecmp (option_effects->list.item[i].s, "glide1") == 0)
			return AnimEffectGlide1;

		else if (strcasecmp (option_effects->list.item[i].s, "glide2") == 0)
			return AnimEffectGlide2;

		else if (strcasecmp (option_effects->list.item[i].s, "horizontalfolds") == 0)
			return AnimEffectHorizontalFolds;

		else if (strcasecmp (option_effects->list.item[i].s, "magiclamp") == 0)
			return AnimEffectMagicLamp;

		else if (strcasecmp (option_effects->list.item[i].s, "rollup") == 0)
			return AnimEffectRollUp;

		else if (strcasecmp (option_effects->list.item[i].s, "sidekick") == 0)
			return AnimEffectSidekick;

		else if (strcasecmp (option_effects->list.item[i].s, "vacuum") == 0)
			return AnimEffectVacuum;

		else if (strcasecmp (option_effects->list.item[i].s, "wave") == 0)
			return AnimEffectWave;
                   ...
                   short      *y)
{
	...
	if (XQueryPointer
	           (display.display, s->root, &w1, &w2, &xj, &yj, &xp, &yp, &m))
	{
		*x = xp;
		*y = yp;
...
static void
cleanUpParentChildChainItem (AnimScreen *as,
                             AnimWindow *aw)
{
...
static void
clearDodgersSubject (AnimScreen *as,
                     CompWindow *w)
{
	CompWindow *dw;
	AnimWindow *adw;
...
static void
removeFromDodgeChain (AnimScreen *as,
                      CompWindow *dw)
{
                   ...
                   Bool       activating)
{
	...
	(*display.handleFusilliEvent) ("animation", "activate", arg, 2);
}
...
initiateFocusAnimation (CompWindow *w)
{
	    ...
	    aw->com.curWindowEvent != WindowEventMinimize)
	{
		    ...
		    chosenEffect == AnimEffectDodge)
		{
			...
			{
				const BananaValue *
				option_dodge_gap_ratio = bananaGetOption (bananaIndex,
				                                          "dodge_gap_ratio",
				                                          s->screenNum);
...
static Bool
restackInfoStillGood (CompScreen  *s,
...
static Bool
wontCreateCircularChain (CompWindow *wCur,
                         CompWindow *wNext)
{
...
static void
animPreparePaintScreen (CompScreen *s,
...
static void
animDrawWindowTexture (CompWindow           *w,
                       CompTexture          *texture,
...
static void
animHandleFusilliEvent (const char      *pluginName,
                        ...
                        int             nArg)
{
	...
	UNWRAP (ad, &display, handleFusilliEvent);
	(*display.handleFusilliEvent) (pluginName, eventName, arg, nArg);
	WRAP (ad, &display, handleFusilliEvent, animHandleFusilliEvent);
	...
	for (i = 0; i < NUM_WATCHED_PLUGINS; i++)
		if (strcmp (pluginName, watchedPlugins[i].pluginName) == 0)
		{
			if (strcmp (eventName, watchedPlugins[i].activateEventName) == 0)
			{
				...
				CompScreen *s = findScreenAtDisplay (xid);

				if (s)
				{
					ANIM_SCREEN (s);

					BananaValue *arg_active = getArgNamed ("active", arg, nArg);

					if (arg_active)
						as->pluginActive[i] = arg_active->b;
					else
						as->pluginActive[i] = FALSE;
...
static inline Bool
shouldIgnoreForAnim (CompWindow *w,
                     Bool       checkPixmap)
{
	ANIM_DISPLAY (&display);
...
animHandleEvent (XEvent *event)
{
	...
	{
	case PropertyNotify:
		if (event->xproperty.atom == display.clientListStackingAtom)
		{
			...
			else                    // X -> Withdrawn
			{
				...
				if (chosenEffect != AnimEffectNone)
				{
					                 ...
					                 (w->screen, &aw->com.icon.x, &aw->com.icon.y))
					{
						...
						{
							const BananaValue *
							option_magic_lamp_open_start_width = bananaGetOption (
							      bananaIndex,
							...
							aw->com.icon.width =
							        MAX (aw->com.icon.width,
							             option_magic_lamp_open_start_width->i);
						}
						else if (aw->com.curAnimEffect == AnimEffectVacuum)
						{
							const BananaValue *
							option_vacuum_open_start_width = bananaGetOption (
							      bananaIndex,
							      "vacuum_open_start_width",
							      w->screen->screenNum);

							aw->com.icon.width =
							        MAX (aw->com.icon.width,
							             option_vacuum_open_start_width->i);
						}
	...
	case ConfigureNotify:
	{
		...
		w = findWindowAtDisplay (ce->window);
		if (!w)
			break;
		if (w->prev)
		{
	...
	WRAP (ad, &display, handleEvent, animHandleEvent);

	switch (event->type)
	{
	case PropertyNotify:
		if (event->xproperty.atom == display.winActiveAtom &&
		    display.activeWindow != ad->activeWindow)
		{
			ad->activeWindow = display.activeWindow;
...
static Bool
animDamageWindowRect (CompWindow *w,
                      Bool       initial,
                      BoxPtr     rect)
{
	Bool status;
...
static void
animWindowMoveNotify (CompWindow *w,
                      int        dx,
                      int        dy,
                      Bool       immediate)
{
                 ...
                 CompDisplay *d)
{
	...
	matchUpdate (&ad->neverAnimateMatch);

	WRAP (ad, d, handleEvent, animHandleEvent);
	WRAP (ad, d, handleFusilliEvent, animHandleFusilliEvent);
                 ...
                 CompDisplay *d)
{
	...
	UNWRAP (ad, d, handleFusilliEvent);
                ...
                CompScreen *s)
{
	...
	const BananaValue *
	option_open_matches = bananaGetOption (bananaIndex,
	                                       "open_matches",
	                                       s->screenNum);

	as->open_count = option_open_matches->list.nItem;
	for (i = 0; i < option_open_matches->list.nItem; i++)
	{
		matchInit (&as->open_match[i]);
		matchAddFromString (&as->open_match[i],
		                    option_open_matches->list.item[i].s);
		matchUpdate (&as->open_match[i]);
	}

	const BananaValue *
	option_close_matches = bananaGetOption (bananaIndex,
	                                        "close_matches",
	                                        s->screenNum);

	as->close_count = option_close_matches->list.nItem;
	for (i = 0; i < option_close_matches->list.nItem; i++)
	{
		matchInit (&as->close_match[i]);
		matchAddFromString (&as->close_match[i],
		                    option_close_matches->list.item[i].s);
		matchUpdate (&as->close_match[i]);
	}

	const BananaValue *
	option_minimize_matches = bananaGetOption (bananaIndex,
	                                           "minimize_matches",
	                                           s->screenNum);

	as->minimize_count = option_minimize_matches->list.nItem;
	for (i = 0; i < option_minimize_matches->list.nItem; i++)
	{
		matchInit (&as->minimize_match[i]);
		matchAddFromString (&as->minimize_match[i],
		                    option_minimize_matches->list.item[i].s);
		matchUpdate (&as->minimize_match[i]);
	}

	const BananaValue *
	option_shade_matches = bananaGetOption (bananaIndex,
	                                        "shade_matches",
	                                        s->screenNum);

	as->shade_count = option_shade_matches->list.nItem;
	for (i = 0; i < option_shade_matches->list.nItem; i++)
	{
		matchInit (&as->shade_match[i]);
		matchAddFromString (&as->shade_match[i],
		                    option_shade_matches->list.item[i].s);
		matchUpdate (&as->shade_match[i]);
	}

	const BananaValue *
	option_focus_matches = bananaGetOption (bananaIndex,
	                                        "focus_matches",
	                                        s->screenNum);

	as->focus_count = option_focus_matches->list.nItem;
	for (i = 0; i < option_focus_matches->list.nItem; i++)
	{
		matchInit (&as->focus_match[i]);
		matchAddFromString (&as->focus_match[i],
		                    option_focus_matches->list.item[i].s);
		matchUpdate (&as->focus_match[i]);
	}
                ...
                CompScreen *s)
{
	...
	for (i = 0; i < as->open_count; i++)
		matchFini (&as->open_match[i]);

	for (i = 0; i < as->close_count; i++)
		matchFini (&as->close_match[i]);

	for (i = 0; i < as->minimize_count; i++)
		matchFini (&as->minimize_match[i]);

	for (i = 0; i < as->shade_count; i++)
		matchFini (&as->shade_match[i]);

	for (i = 0; i < as->focus_count; i++)
		matchFini (&as->focus_match[i]);
                ...
                CompWindow *w)
{
	...
	w->privates[as->windowPrivateIndex].ptr = NULL;
}

static void
animChangeNotify (const char        *optionName,
                  ...
                  int               screenNum)
{
	...
	if (strcasecmp (optionName, "open_matches") == 0)
	{
		int i;
		for (i = 0; i < as->open_count; i++)
			matchFini (&as->open_match[i]);

		as->open_count = optionValue->list.nItem;
		for (i = 0; i < as->open_count; i++)
		{
			matchInit (&as->open_match[i]);
			matchAddFromString (&as->open_match[i],
	...
	else if (strcasecmp (optionName, "close_matches") == 0)
	{
		int i;
		for (i = 0; i < as->close_count; i++)
			matchFini (&as->close_match[i]);

		as->close_count = optionValue->list.nItem;
		for (i = 0; i < as->close_count; i++)
		{
			matchInit (&as->close_match[i]);
			matchAddFromString (&as->close_match[i],
	...
	else if (strcasecmp (optionName, "minimize_matches") == 0)
	{
		int i;
		for (i = 0; i < as->minimize_count; i++)
			matchFini (&as->minimize_match[i]);

		as->minimize_count = optionValue->list.nItem;
		for (i = 0; i < as->minimize_count; i++)
		{
			matchInit (&as->minimize_match[i]);
			matchAddFromString (&as->minimize_match[i],
	...
	else if (strcasecmp (optionName, "shade_matches") == 0)
	{
		int i;
		for (i = 0; i < as->shade_count; i++)
			matchFini (&as->shade_match[i]);

		as->shade_count = optionValue->list.nItem;
		for (i = 0; i < as->shade_count; i++)
		{
			matchInit (&as->shade_match[i]);
			matchAddFromString (&as->shade_match[i],
	...
	else if (strcasecmp (optionName, "focus_matches") == 0)
	{
		int i;
		for (i = 0; i < as->focus_count; i++)
			matchFini (&as->focus_match[i]);

		as->focus_count = optionValue->list.nItem;
		for (i = 0; i < as->focus_count; i++)
		{
			matchInit (&as->focus_match[i]);
			matchAddFromString (&as->focus_match[i],
...
animInit (CompPlugin *p)
{
	...
	bananaIndex = bananaLoadPlugin ("animation");

	if (bananaIndex == -1)
		return FALSE;

	bananaAddChangeNotifyCallBack (bananaIndex, animChangeNotify);
...
static CompPluginVTable animVTable = {
	"animation",
	animInit,
	animFini,
	animInitDisplay,
