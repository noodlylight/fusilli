...
fxDominoInit (CompWindow *w)
{
	...
	const BananaValue *
	option_razr_direction = bananaGetOption (bananaIndex,
	                                         "razr_direction",
	                                         w->screen->screenNum);

	const BananaValue *
	option_domino_direction = bananaGetOption (bananaIndex,
	                                           "domino_direction",
	                                           w->screen->screenNum);

	if (isRazr)
		fallDir = animBaseFunctions.getActualAnimDirection
		                  (w, option_razr_direction->i, TRUE);
	else
		fallDir = animBaseFunctions.getActualAnimDirection
		                  (w, option_domino_direction->i, TRUE);
