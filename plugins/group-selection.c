...
{
	const BananaValue *
	option_select_precision = bananaGetOption (bananaIndex,
	                                           "select_precision",
	                                           s->screenNum);

	float precision = option_select_precision->i / 100.0f;
	CompWindow **ret = NULL;
	int count = 0;
	CompWindow *w;
			...
			if (gs->x1 != gs->x2 && gs->y1 != gs->y2)
			{
				...
				if (reg)
				{
					...
					if (ws)
					{
						...
						const BananaValue *
						option_auto_group = bananaGetOption (bananaIndex,
						                                     "auto_group",
						                                     s->screenNum);

						if (option_auto_group->b)
							groupGroupWindows (0);
