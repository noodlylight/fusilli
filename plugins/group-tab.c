                              ...
                              int        *y)
{
	...
	result = XQueryPointer (display.display, s->root, &root,
                            ...
                            GroupWindowHideInfo *hideInfo)
{
	...
	rects = XShapeGetRectangles (display.display, w->id, ShapeInput,
	                             &count, &ordering);
                          ...
                          Bool       visible)
{
	...
	else if (visible && gw->windowHideInfo)
	{
		...
		if (info->nInputRects)
		{
			XShapeCombineRectangles (display.display, w->id, ShapeInput, 0, 0,
			                         info->inputRects, info->nInputRects,
			                         ShapeSet, info->inputRectOrdering);
		}
		else
		{
			XShapeCombineMask (display.display, w->id, ShapeInput,
			                   0, 0, None, ShapeSet);
		}
		...
		XShapeSelectInput (display.display, w->id, info->shapeMask);
                       ...
                       unsigned int   mask)
{
	...
	else if (visible && (bar->state == PaintOff || bar->state == PaintFadeOut))
	{
		...
		if (option_bar_animations->b)
		{
			...
			bar->bgAnimationTime = option_reflex_time->f * 1000.0;
		}
	...
	if (bar->state == PaintFadeIn || bar->state == PaintFadeOut)
		bar->animationTime = (option_fade_time->f * 1000) - bar->animationTime;
                             ...
                             Bool       activating)
{
	...
	(*display.handleFusilliEvent) ("group", "tabChangeActivate", arg, 2);
}
...
groupHandleAnimation (GroupSelection *group)
{
	...
	const BananaValue *
	option_visibility_time = bananaGetOption (bananaIndex,
	                                          "visibility_time",
	                                          s->screenNum);
	...
	if (group->changeState == TabChangeOldOut)
	{
		...
		group->changeAnimationTime += option_change_animation_time->f * 500;
	    ...
	    group->changeAnimationTime <= 0)
	{
		...
		else if (option_visibility_time->f != 0.0f &&
		         group->changeState == NoTabChange)
		{
			...
			group->tabBar->timeoutHandle =
			        compAddTimeout (option_visibility_time->f * 1000,
			                        option_visibility_time->f * 1200,
                       ...
                       int            msSinceLastPaint)
{
	...
	const BananaValue *
	option_tabbing_speed = bananaGetOption (bananaIndex,
	                                        "tabbing_speed",
	                                        s->screenNum);

	const BananaValue *
	option_tabbing_timestep = bananaGetOption (bananaIndex,
	                                           "tabbing_timestep",
	                                           s->screenNum);

	amount = msSinceLastPaint * 0.05f * option_tabbing_speed->f;
	steps = amount / (0.5f * option_tabbing_timestep->f);
	if (!steps)
		steps = 1;
                    ...
                    Window     enteredWin)
{
	    ...
	    !TOP_TAB (hoveredGroup)->grabbed)
	{
		...
		{
			const BananaValue *
			option_tabbar_show_delay = bananaGetOption (bananaIndex,
			                                            "tabbar_show_delay",
			                                            s->screenNum);

			int showDelayTime = option_tabbar_show_delay->f * 1000;
...
groupTabGroup (CompWindow *main)
{
	...
	if (group->tabBar->textLayer)
	{
		...
		layer = group->tabBar->textLayer;
		layer->animationTime = option_fade_text_time->f * 1000;
		layer->state = PaintFadeIn;
	}

	/* we need a buffer for DnD here */
	const BananaValue *
	option_thumb_space = bananaGetOption (bananaIndex,
	                                      "thumb_space",
	                                      s->screenNum);
	...
	space = option_thumb_space->i;
	thumbSize = option_thumb_size->i;
	...
	if (group->changeState != NoTabChange)
	{
		if (group->prevTopTab == topTab)
		{
			...
			group->changeAnimationTime =
			        option_change_animation_time->f * 500 -
			        group->changeAnimationTime;
                      ...
                      int            maxX2)
{
	...
	/* just a little work-a-round for first call
	   FIXME: remove this! */
	thumbSize = option_thumb_space->i;
                       ...
                       Bool            temporary)
{
	...
	if (!temporary)
	{
		...
		if (IS_TOP_TAB (w, group))
		{
			...
			const BananaValue *
			option_untab_on_close = bananaGetOption (bananaIndex,
			                                         "untab_on_close",
			                                         s->screenNum);

			if (option_untab_on_close->b)
                       ...
                       int        distanceY)
{
	...
	const BananaValue *
	option_drag_spring_k = bananaGetOption (bananaIndex,
	                                        "drag_spring_k",
	                                        s->screenNum);

	/* The maximum value */
	float a = option_drag_spring_k->f * 
	          (option_thumb_size->i + option_border_radius->i) / 2;
	/* This will make distanceX == 2 * (SIZE + BORDER) to get 0,
	   and distanceX == (SIZE + BORDER) to get the maximum. */
	float b = PI /  (2 * option_thumb_size->i + 2 * option_border_radius->i);
	...
	const BananaValue *
	option_drag_y_distance = bananaGetOption (bananaIndex,
	                                          "drag_y_distance",
	                                          s->screenNum);

	if (distanceY < option_drag_y_distance->i)
		a *= 1.0f - (float)distanceY / option_drag_y_distance->i;
	else
		a = 0;

	if (abs (distanceX) < 2 * (option_thumb_size->i + option_border_radius->i))
                  ...
                  GroupTabBarSlot *draggedSlot)
{
	...
	if (draggedSlot)
	{
		...
		leftForce = groupDraggedSlotForce (s,
		                                   bar->region->extents.x1 -
		                                   option_thumb_size->i / 2 - draggedCenterX,
	...
	const BananaValue *
	option_drag_speed_limit = bananaGetOption (bananaIndex,
	                                           "drag_speed_limit",
	                                           s->screenNum);

	const BananaValue *
	option_drag_friction = bananaGetOption (bananaIndex,
	                                        "drag_friction",
	                                         s->screenNum);

	for (slot = bar->slots; slot; slot = slot->next)
	{
		groupApplyFriction (s, &slot->speed, option_drag_friction->f);
		groupApplySpeedLimit (s, &slot->speed, option_drag_speed_limit->i);
	}

	groupApplyFriction (s, &bar->leftSpeed, option_drag_friction->f);
	groupApplySpeedLimit (s, &bar->leftSpeed, option_drag_speed_limit->i);

	groupApplyFriction (s, &bar->rightSpeed, option_drag_friction->f);
	groupApplySpeedLimit (s, &bar->rightSpeed, option_drag_speed_limit->i);
}
                  ...
                  int            msSinceLastRepaint)
{
	...
	if (move)
	{
		...
		updateTabBar = TRUE;
	         ...
	         (option_drag_spring_k->f * 
	          abs (bar->region->extents.x1 - bar->leftSpringX) <
	          option_drag_friction->f))
	{
	...
	if (move)
	{
		...
		updateTabBar = TRUE;
	         ...
	         (option_drag_spring_k->f * 
	          abs (bar->region->extents.x2 - bar->rightSpringX) <
	          option_drag_friction->f))
	{
	...
	for (slot = bar->slots; slot; slot = slot->next)
	{
		...
		if (move)
		{
			...
			slot->msSinceLastMove = 0;
		         ...
		         option_drag_spring_k->f * 
		         abs (slotCenter - slot->springX) < option_drag_friction->f)
		{
 ...
 */
Bool
groupChangeTabLeft (Window xid)
{
	CompWindow *w, *topTab;

	w   = topTab = findWindowAtDisplay (xid);
	if (!w)
		return TRUE;
 ...
 */
Bool
groupChangeTabRight (Window xid)
{
	CompWindow *w, *topTab;
