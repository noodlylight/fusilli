...
static Bool
processIntersectingPolygons (CompScreen *s,
                             PolygonSet *pset)
{
	int j;
                             ...
                             CompTransform *matf)
{
	...
	GLfloat skewx = -(((center.x - as->animaddon_output->region.extents.x1) -
	                   as->animaddon_output->width / 2) * 1.15);
	GLfloat skewy = -(((center.y - as->animaddon_output->region.extents.y1) -
	                   as->animaddon_output->height / 2) * 1.15);
...
polygonsDrawCustomGeometry (CompWindow *w)
{
	...
	float forwardProgress = animBaseFunctions.defaultAnimProgress (w);
	...
	if (pset->allFadeDuration > -1.0f)
	{
		...
		if (fadePassedBy > 1e-5) // if true, allFadeDuration should be > 0
		{
			...
			if (aw->deceleratingMotion)
				opacityFac = 1 - animBaseFunctions.decelerateProgress
			     ...
			     i++, nFrontVerticesTilThisPoly += nNewSides)
			{
				...
				if (pset->allFadeDuration == -1.0f)
				{
					...
					if (fadePassedBy > 1e-5) // if true, then allFadeDuration > 0
					{
						...
						if (aw->deceleratingMotion)
							opacityFac =
							        1 - animBaseFunctions.decelerateProgress
                                     ...
                                     float         forwardProgress)
{
	...
	moveProgress = animBaseFunctions.decelerateProgress (moveProgress);
...
getAnimStepPolygonFunc (AnimWindow *aw)
{
                  ...
                  float      time)
{
	...
	animBaseFunctions.defaultAnimStep (w, time);
	...
	if (aw->eng.polygonSet)
	{
		...
		compLogMessage ("animation", CompLogLevelDebug,
                  ...
                  Box        *BB)
{
	...
	animBaseFunctions.prepareTransform (s, output, &wTransform, &wTransform2);
...
#define N_POINTS 8
		...
		for (j = 0; j < N_POINTS; j++, pnt++)
		{
			...
			py = s->height - py;
			animBaseFunctions.expandBoxWithPoint (BB, px + 0.5, py + 0.5);
		}
#undef N_POINTS
	}
}
