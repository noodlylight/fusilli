...
fxBeamUpInit (CompWindow *w)
{
	...
	animBaseFunctions.defaultAnimInit (w);
	...
	const BananaValue *
	option_beam_slowdown = bananaGetOption (bananaIndex,
	                                        "beam_slowdown",
	                                        w->screen->screenNum);

	aw->eng.ps[0].slowdown = option_beam_slowdown->f;
...
{
	const BananaValue *
	option_beam_spacing = bananaGetOption (bananaIndex,
	                                       "beam_spacing",
	                                       w->screen->screenNum);
	...
	ps->numParticles =
	        width / option_beam_spacing->i;

	float beaumUpLife = option_fire_life->f;
	...
	const BananaValue *
	option_beam_color = bananaGetOption (bananaIndex,
	                                     "beam_color",
	                                     w->screen->screenNum);

	unsigned short c[] = { 0, 0, 0, 0 };

	stringToColor (option_beam_color->s, c);
	...
	const BananaValue *
	option_beam_size = bananaGetOption (bananaIndex,
	                                    "beam_size",
	                                    w->screen->screenNum);

	float partw = 2.5 * option_beam_size->f;
